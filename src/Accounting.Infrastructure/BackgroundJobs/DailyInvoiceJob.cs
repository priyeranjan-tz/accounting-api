...
/// <summary>
/// Background job for generating daily invoices
...
public class DailyInvoiceJob : IJob
{
    ...
    public DailyInvoiceJob(
        IServiceProvider serviceProvider,
        ILogger<DailyInvoiceJob> logger)
    {
    ...
    public async Task Execute(IJobExecutionContext context)
    {
        ...
        _logger.LogInformation(
            "DailyInvoiceJob started - JobId: {JobId}, ScheduledTime: {ScheduledTime}",
            jobId, scheduledTime);
        ...
        try
        {
            ...
            {
                _logger.LogInformation(
                    "DailyInvoiceJob completed successfully - JobId: {JobId}, Duration: {Duration}ms, " +
            ...
            {
                _logger.LogError(
                    "DailyInvoiceJob failed - JobId: {JobId}, Duration: {Duration}ms, Error: {Error}",
        ...
        catch (Exception ex)
        {
            var duration = DateTime.UtcNow - startTime;
            _logger.LogError(ex,
                "DailyInvoiceJob failed with exception - JobId: {JobId}, Duration: {Duration}ms",
                jobId, duration.TotalMilliseconds);
            throw;
        }
    }
