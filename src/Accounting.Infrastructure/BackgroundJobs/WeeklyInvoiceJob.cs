...
/// <summary>
/// Background job for generating weekly invoices
/// Runs every Sunday at midnight UTC
/// </summary>
[DisallowConcurrentExecution]
public class WeeklyInvoiceJob : IJob
{
    private readonly IServiceProvider _serviceProvider;
    private readonly ILogger<WeeklyInvoiceJob> _logger;

    public WeeklyInvoiceJob(
        IServiceProvider serviceProvider,
        ILogger<WeeklyInvoiceJob> logger)
    {
    ...
    public async Task Execute(IJobExecutionContext context)
    {
        ...
        _logger.LogInformation(
            "WeeklyInvoiceJob started - JobId: {JobId}, ScheduledTime: {ScheduledTime}",
            jobId, scheduledTime);
        ...
        try
        {
            ...
            {
                _logger.LogInformation(
                    "WeeklyInvoiceJob completed successfully - JobId: {JobId}, Duration: {Duration}ms, " +
            ...
            {
                _logger.LogError(
                    "WeeklyInvoiceJob failed - JobId: {JobId}, Duration: {Duration}ms, Error: {Error}",
        ...
        catch (Exception ex)
        {
            var duration = DateTime.UtcNow - startTime;
            _logger.LogError(ex,
                "WeeklyInvoiceJob failed with exception - JobId: {JobId}, Duration: {Duration}ms",
                jobId, duration.TotalMilliseconds);
            throw;
        }
    }
